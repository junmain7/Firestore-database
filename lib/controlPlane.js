...
// Reversible on/off switch — separate from revoke (which is permanent).
// Lets an admin cut a customer's access off and turn it back on later
// (e.g. non-payment) without losing the key/history.
async function setKeyEnabled(keyHash, enabled) {
  const db = controlDb();
  await db.collection(KEYS_COL).doc(keyHash).update({ disabled: !enabled });
}
...
async function addAdminDoc({ email, addedBy, role }) {
  ...
  await db.collection(ADMINS_COL).doc(id).set({
    email: id,
    role: normalizedRole,
    keyAccess: normalizedRole === "admin", // users need explicit grant before they can create keys
    disabled: false, // suspend switch — invalidates their existing keys without removing them
...
async function getUserDoc(email) {
  const db = controlDb();
  const doc = await db.collection(ADMINS_COL).doc(email.toLowerCase()).get();
  return doc.exists ? doc.data() : null;
}

async function setUserKeyAccess(email, granted) {
  const db = controlDb();
  await db.collection(ADMINS_COL).doc(email.toLowerCase()).update({ keyAccess: !!granted });
}

async function setUserDisabled(email, disabled) {
  const db = controlDb();
  await db.collection(ADMINS_COL).doc(email.toLowerCase()).update({ disabled: !!disabled });
}
